...
export default function LineProgressBarBlock({
	...
	if (isCentered) {
		return (
			<div className={clsx("p-3 border border-secondaryLight rounded-3xl w-full", additionalClasses)}>
				<div className="flex justify-center relative">
					<span className={clsx(" text-xl font-extrabold leading-[110%]", {
                        'text-dark': percent !== 51 && percent <= 100,
                        'text-negative': percent === 51,
                        'text-positive': percent > 100
                    })}>
						{percent.toString().replace(/[.]/g, ",")}%
					</span>
					{increasePercent && (
						<div className="flex items-center gap-0.5 py-0.5 px-1 bg-positive text-white rounded-3xl absolute right-0 top-0">
							<svg
				...
				<div className="w-full rounded-xl bg-secondaryLight h-1 my-4">
					<div
				...
				<div className="flex flex-col items-center gap-2">
					<h6 className="text-secondaryDark text-sm font-bold text-center">
						{label}
					</h6>
					<p className="text-xs text-gray font-medium text-center">
						{opinion}
					</p>
				</div>
			</div>
			...
			<div
				...
				<div className="flex justify-between">
					<span className="text-dark text-xl font-extrabold leading-[110%]">
						{percent.toString().replace(/[.]/g, ",")}{!isDisableProgressBar && '%'}
					</span>
					{increasePercent && (
				...
				{!isDisableProgressBar && <div className="w-full rounded-xl bg-secondaryLight h-1 my-3">
					<div
						className={`rounded-xl h-full ${
							!isNegative
								? "bg-gradient-positive"
				...
				<div className={clsx({
                    'mt-3': isDisableProgressBar
                })}>
					<h6 className="text-secondaryDark text-xs font-bold">
						{label}
					</h6>
