...
export default function PlatformLayout() {
	...
	useEffect(() => {
		setCanGrowHeight(
			href.split("/")[2] === "profile" || href.split("/")[2] === "metrics",
		);
	}, [href]);
	...
	if (auth === "yes") {
		return (
			<>
				...
				<div
					className={clsx(
						"flex gap-3 bg-[#F9FAFC] w-screen min-h-screen p-3 lg:flex-col",
						!canGrowHeight && "items-start",
					)}
				>
					...
					<div
						className={clsx(
							"shadow-darken bg-white p-5 rounded-3xl grow w-full relative md:rounded-xl max-w-full overflow-hidden",
							!canGrowHeight && "self-start",
						)}
					>
						<img
